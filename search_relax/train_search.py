...
def print_genotype(geno):
    ...
    geno_out = [(sub_policy[0][3], tuple([(x[0], x[1], x[2]) for x in sub_policy])) for sub_policy in geno]
    logging.info("genotype_p_%d: %s" % ( len(geno_out), str(geno_out) ))
    logging.info("genotype_p_%d: %s" % ( len(geno_out[0:5]), str(geno_out[0:5]) ))
    logging.info("genotype_p_%d: %s" % ( len(geno_out[0:10]), str(geno_out[0:10]) ))
    logging.info("genotype_p_%d: %s" % ( len(geno_out[0:15]), str(geno_out[0:15]) ))
    logging.info("genotype_p_%d: %s" % ( len(geno_out[0:20]), str(geno_out[0:20]) ))
    logging.info("genotype_p_%d: %s" % ( len(geno_out[0:25]), str(geno_out[0:25]) ))
