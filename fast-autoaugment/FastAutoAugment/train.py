...
def train_and_eval(tag, dataroot, test_ratio=0.0, cv_fold=0, reporter=None, metric='best', save_path=None, only_eval=False, horovod=False):
    if horovod:
    ...
    for epoch in range(epoch_start, max_epoch + 1):
        ...
        if epoch % 1 == 0 or epoch == max_epoch:
            ...
            }, save_path.replace('.pth', '_last.pth'))
...
if __name__ == '__main__':
    ...
    if not os.path.exists(os.path.dirname(args.save)):
        os.mkdir(os.path.dirname(args.save))
    # log_format = '%(asctime)s %(message)s'
    # logging.basicConfig(stream=sys.stdout, level=logging.INFO,
    #                     format=log_format, datefmt='%m/%d %I:%M:%S %p')
    formatter = logging.Formatter('[%(asctime)s] [%(name)s] [%(levelname)s] %(message)s')
    fh = logging.FileHandler(os.path.join(os.path.dirname(args.save), 'log-%s.txt' % (time.strftime("%Y%m%d-%H%M%S"), ) ))
